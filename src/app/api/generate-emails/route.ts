...
export async function POST(request: Request) {
  try {
    ...
    // 6. Contact Anthropic Claude API
    ...
    if (!claudeResponse.ok) {
      ...
      return NextResponse.json(
        { error: `Claude API returned an error: ${errorText}` },
        { status: 502 }
      );
    }

    const claudeData = await claudeResponse.json();
    let textOutput = claudeData.content[0].text.trim();

    // Strip markdown code fences if Claude wraps the JSON output
    ...
    try {
      ...
      return NextResponse.json(
        { error: "Failed to parse JSON output generated by Claude." },
        { status: 502 }
      );
    }
    ...
    if (!Array.isArray(parsedEmails) || parsedEmails.length !== 7) {
      return NextResponse.json(
        { error: "Claude did not return exactly 7 onboarding emails." },
        { status: 502 }
      );
    }
